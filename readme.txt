...
* 新增子外掛，預設使用 `admin-staff` 關鍵字隱藏網站登入位置。路徑可以使用 `MDT_LOGIN_PATH` 常數於 `wp-config.php` 中覆寫。
...
* 參考 [Gist](https://gist.github.com/Viper007Bond/5192117) 方法，新增統計 Hooks 勾點的呼叫執行時間排名。使用 `debug=1` Query String 請求參數來查看，效能考量，預設不啟用。無 admin bar 的情況下，新增瀏覽器開發者模式 console 分頁觀看。

= 2.7.4 =

* 新增 `mxp_dev_default_exclude_dirs` 事件過濾（filter）方法，預設打包排除一些快取或是其他備份外掛目錄
...
* 調整打包 `wp-content` 全目錄功能的實作方式，避免請求超時問題。
* 補上清除暫存檔案的功能
* 修正一些小錯誤

= 2.4.3 =

* 新增打包 `wp-content` 全目錄功能。
...
* 加入查詢網站最近變動過檔案的功能，尤其掃描系統隱藏檔案，確保可以清楚得知可能被駭的檔案。（預設這操作會刪除網站資料夾內 Mac 上常見的無用檔案 `.DS_Store `）
...
* 加入 WordPress 最佳化設定資料表 `*_options` 的工具
* 修正操作介面
...
* 新增資料庫取代字串功能，此功能以 interconnect\it 的 [Search Replace DB](https://github.com/interconnectit/Search-Replace-DB/) 為基礎修改的功能。
* 修正搜尋與取代的畫面操作使用體驗。
